...
import { ChatDto, message } from './dto/chat.dto';
  ...
  implements OnModuleInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleCreationOfTheRoom(
    ...
    if (User) {
      ...
      if (!found) {
        ...
        if (
          newRoom.isDm &&
          (User.blocks.find((user) => user.email == Body.email[0]) ||
  ...
  async validateUserByEmail(email, roomName, num) {
    const user = await this.prisma.user.findUnique({
      where: { email: email },
      include: { blocks: true, blockedBy: true },
    });
    const room = await this.prisma.chatRoom.findFirst({
      where: { roomName: roomName },
    });
    if (num == 0) return user;
    return room?.bannedUsers?.indexOf(user.email) < 0 ? user : undefined;
  }
}