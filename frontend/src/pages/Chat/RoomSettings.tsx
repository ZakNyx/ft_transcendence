...
const ParticipantOwner = (props:any) => {
    ...
    return (
    <div key={props.index} className='w-full h-full flex-wrap justify-around items-center'>
...
const ParticipantUser = (props: any) => {
    ...
    const role = props.roomState.rooms[0].role;
    return (
    <div key={props.key} className='w-full h-full flex-wrap justify-around items-center'>
        <div className="w-full h-full flex p-[auto] items-center justify-start">
            <img
                className="logoImg rounded-[50px] ml-[30px] w-[40px] h-[40px] mr-[25px]"
                src={props.roomState.image}
...
const RoomSettings = (props:any) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
            const response = await axios.get(`http://localhost:3000/chat/roomSettings/${receivedData}`, {
                params: {
                    userId: userId,
                },
                headers: {
    ...
    fetchData();
        ...
        socket.on("banned", (bannedId: string) => 
        {
            if (userId === bannedId)
            {
                const path: string = "http://localhost:5173/chat/roomSettings/?id=" + receivedData;
                        ...
                        roomState.participants.map( (participant: any) => {
                            ...
                            if(roomState.role === "USER")
                            {
                                return(
                                    ...
                                    {
                                        <ParticipantUser index={participant.id} roomState={participant} userId={props.userId} role={roomState.role}/> 
                                    }
                                    </div>
                                )
                            }
                                ...
                                return(
                                    <div key={participant.id} className="w-full h-full">
                                    {
                                        <ParticipantOwner  index={participant.id} roomState={participant} userId={props.userId} role={roomState.role} socket={props.socket}/> 
                                    }
                                    </div>
                                )
                                ...
                                return(
                                    <div key={participant.id} className="w-full h-full">
                                    {
                                        <ParticipantAdmin  index={participant.id} roomState={participant} userId={props.userId} role={roomState.role} socket={props.socket}/> 
                                    }
                                    </div>
                                )
                                ...
                                return(
                                    <div key={participant.id} className="w-full h-full">
                                    {
                                        <ParticipantAdmin1 index={participant.id} roomState={participant} userId={props.userId} role={roomState.role} socket={props.socket}/> 
                                    }
                                    </div>
                                )
