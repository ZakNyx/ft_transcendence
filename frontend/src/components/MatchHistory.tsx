  ...
  const MatchHistory = () => {
    ...
    useEffect(() => {
      if (gameData && gameData.length > 0) {
        const firstOpponentImage = gameData[0]?.users?.[0]?.image || null;
        setOpponentImage(firstOpponentImage);
      }
    }, [gameData]); 

    if(gameData)
    {
      console.log('check gameData : ', gameData );
        ...
        <ul className="space-y-4">
          {gameData && gameData.map((match: any, index: any) => (
            ...
            >
              ...
              <div className="flex items-center space-x-2">
                <img
                  src={opponentImage || "../../public/images/default.png"}
                  alt={`Your profile`}
                  className="w-12 h-12 rounded-full"
                />
                <span className="text-gray-200 text-base">{match.player1 != Playerusername? match.player1 : match.player2}</span>
